...
function ProcessFlow({ opportunity, customer, quoteId }) {
  ...
  return (
    <div className="flex items-stretch gap-0 flex-wrap">
      {steps.map((step, idx) => (
        <div key={idx} className="flex items-center flex-1 min-w-[160px]">
          {/* Card */}
          <div className={`flex-1 rounded-xl border-2 ${step.border} ${step.bg} p-4 shadow-sm`}>
            <div className="flex items-center gap-2 mb-2">
              <div className={`w-8 h-8 rounded-lg ${step.iconBg} flex items-center justify-center text-white font-bold text-sm`}>
                {step.icon}
              </div>
            ...
            <p className="text-sm text-gray-600 mt-0.5 font-medium truncate">{step.detail}</p>
          ...
          {idx < steps.length - 1 && (
            <div className="flex flex-col items-center mx-2 flex-shrink-0">
              <div className="pulse-arrow">
                <ArrowRightFilled fontSize={22} className="text-[#DDDBDA]" />
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#F3F2F2] font-sans">
      ...
      <div className="fade-in-up mx-6 mt-5">
        <div className="bg-[#EBF5E9] border border-[#2E844A]/30 rounded-lg px-5 py-4 flex items-center gap-4 shadow-sm">
          <CheckmarkCircleFilled fontSize={28} className="text-[#2E844A] flex-shrink-0" />
      ...
      <div className="px-6 py-5 grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div className="xl:col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-lg border border-[#DDDBDA] shadow-sm overflow-hidden">
            {/* SF blue accent (identifies this as Salesforce context) */}
            <div className="h-1 bg-[#0176D3]" />
            <div className="p-5">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="flex items-start gap-3">
                  <div className="w-11 h-11 bg-[#EBF5FF] rounded-lg flex items-center justify-center flex-shrink-0 border border-[#0176D3]/20">
                    <DocumentRegular fontSize={22} className="text-[#0176D3]" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className="text-[10px] text-[#706E6B] font-semibold uppercase tracking-wider">Quotation</p>
                      {/* SAP origin badge — shows data comes from SAP S/4HANA */}
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold bg-[#0854A0] text-white">
                        SAP S/4HANA
                      </span>
                    </div>
                    <h1 className="text-xl font-bold text-[#3E3E3C]">{quoteId}</h1>
                    <p className="text-xs text-[#706E6B] mt-0.5">{opportunity.title}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-[#706E6B] font-semibold uppercase tracking-wider">Status</p>
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#EBF5E9] text-[#2E844A]
                                   rounded-full text-xs font-bold border border-[#2E844A]/20 mt-1">
                    <CheckmarkCircleFilled fontSize={12} /> Created
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-4 mt-5 pt-5
                              border-t border-[#DDDBDA] text-xs">
                {[
                  ...
                  <div key={label}>
                    ...
                    <p className="font-semibold text-[#3E3E3C] text-xs">{value}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Line items table */}
          <div className="bg-white rounded-lg border border-[#DDDBDA] shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-[#DDDBDA] bg-[#F3F2F2] flex items-center justify-between">
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-[#F3F2F2] border-b border-[#DDDBDA]">
                    {['Item', 'Material', 'Category', 'Quantity', 'Unit Price', 'Net Value'].map((h) => (
                      <th
                        key={h}
                        className="text-left px-4 py-2.5 text-[10px] font-semibold text-[#706E6B] uppercase tracking-wider"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  {lineItems.map((item, idx) => (
                    <tr
                      key={item.id}
                      className={`border-b border-[#DDDBDA]/60 ${idx % 2 === 0 ? 'bg-white' : 'bg-[#FAFAF9]'}`}
                    >
                      <td className="px-4 py-3 text-[#706E6B] font-mono">
                        {String(idx + 1).padStart(3, '0')}0
                      </td>
                      <td className="px-4 py-3">
                        <p className="font-semibold text-[#3E3E3C]">{item.name}</p>
                        <p className="text-[#A8A8A8] mt-0.5">Mat. {item.id.toUpperCase()}-{2400 + idx}</p>
                      </td>
                      <td className="px-4 py-3 text-[#706E6B]">{item.category}</td>
                      <td className="px-4 py-3 font-semibold text-[#3E3E3C]">
                        {fmtQty(item.quantity, item.uom)}
                      </td>
                      <td className="px-4 py-3 text-[#3E3E3C] font-mono">
                        {fmt(item.unit_price)}
                        <span className="text-[#A8A8A8] ml-1">/{item.uom === 'KG' ? 'KG' : 'MT'}</span>
                      </td>
                      <td className="px-4 py-3 font-bold text-[#3E3E3C] font-mono">
                        {fmt(item.total)}
                      </td>
                    </tr>
                  ))}
                ...
                <tfoot>
                  <tr className="bg-[#F3F2F2] border-t-2 border-[#0176D3]/20">
                    <td colSpan={5} className="px-4 py-3 text-right font-semibold text-[#706E6B] text-xs">
                      Total
                    </td>
                    <td className="px-4 py-3 font-extrabold text-[#3E3E3C] font-mono text-sm">
                      {fmt(grandTotal)}
                    </td>
                  </tr>
                </tfoot>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-lg border border-[#DDDBDA] shadow-sm p-5">
            <h3 className="text-xs font-bold text-[#3E3E3C] mb-4 pb-2 border-b border-[#DDDBDA] flex items-center gap-2 uppercase tracking-wide">
              <CheckmarkCircleFilled fontSize={15} className="text-[#2E844A]" />
              End-to-End Flow
            </h3>
          ...
          <div className="bg-[#0176D3] rounded-lg p-5 text-white shadow-md">
            ...
            <p className="text-white/60 text-xs mt-1">{lineItems.length} line items · EUR</p>
            <div className="mt-4 pt-4 border-t border-white/20 grid grid-cols-2 gap-3 text-xs">
              ...
              <div>
                <p className="text-white/60 text-[10px]">Valid Until</p>
                <p className="font-bold">{validStr}</p>
              </div>
              <div>
                <p className="text-white/60 text-[10px]">Customer</p>
                <p className="font-bold truncate">{customer.name}</p>
              </div>
              <div>
                <p className="text-white/60 text-[10px]">AI Priced</p>
          ...
          {/* Next steps */}
          <div className="bg-white rounded-lg border border-[#DDDBDA] shadow-sm p-5">
            <h3 className="text-xs font-bold text-[#3E3E3C] mb-3 uppercase tracking-wide">Next Steps</h3>
